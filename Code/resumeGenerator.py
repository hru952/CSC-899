...
def generate_resume_bullet_point(text):
    ...
    data = {
        'prompt': f"This is a summary of a computer science student project or class assignment: {text} \n. Generate resume bullet points based on the above summary, following these rules: Give a title; generate 2 to 3 bullet points with length no more than 2 lines or 20 - 30 words per bullet point ; Use STAR format; don't use of any responsibility-oriented language, such as 'Responsible for' or 'Duties included', on the resume. \n Note that the summary provided is generated by a summarization tool and may not cover everything so generate bullet points accordingly.",
        'max_tokens': 300,  # You can adjust the max tokens based on the desired length of the response
    }
